(function() {
  ...
  /**
   * Tiny commonjs modules implementation.
   */
  var modules = {};

  var define = function(name, dependencies, module) {
    if (!module) {
      module = dependencies;
      dependencies = [];
    }

    modules[name] = { name: name, deps: dependencies, module: module };
  };

  var require = function(name) {
    var module = modules[name];
    if (!module) {
      throw "Could not find module named " + name;
    }

    if (!module.exports) {
      var args = module.deps.map(function(dep) { return require(dep); });
      module.exports = module.module.apply(null, args);
    }
   ...
   */
  define('StyleTransaction', function() {
   ...
   */
  define("SlidePage", ["StyleTransaction"], function(StyleTransaction) {

    function SlidePage(node, opts, done) {
  ...
  define("server", function() {
    ...
    return { createPass: createPass };
  });

  define('main', ['server', 'SlidePage'], function(server, SlidePage) {
    ...
    new SlidePage(div, { direction: 'right', amount: 300, duration: 500 }, function(s) {
      setTimeout(function() {
        s.slideBack();
      }, 2000);
    });
