...
/**
 * Zips all the files together.
 * TODO: Needs to create a manifest and sign it.
 */
...
/**
 * Builds an in-memory pass. Compiles the json. Renders files from
 * multiple sources.
 */
...
exports.createPass = function(provider, data, cb) {
  ...
  provider.createPass(data, function(err, pass) {
    ...
    signPass(serial + ".pkpass", pass.files, function() {
      model.Pass({serial: serial}).save(function(err, pass) {
        cb(err, pass);
      });
    });
  });
