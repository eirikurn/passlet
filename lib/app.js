...
/**
 * Render css files using ejs for variable interpolation.
 * With or without minification.
 */
app.engine('css', function(file, opts, cb) {
  ejs.renderFile(file, opts, function(e, css) {
    if (e)
      return cb(e);
...
app.locals.escapejs = function(str) {
  return str.replace(/[^a-zA-Z0-9 *=\/]/g, function(char) {
    ...
    if (code < 256) {
      return "\\x" + ("00".substring(hex.length)) + hex.toUpperCase();
    } else {
...
app.render('scrape.css', function(e, css) {
  if (e) {
    console.log("ERROR: Compiling scraper css.", e);
    return process.exit(1);
  }
  app.locals.scrapecss = css;
});
